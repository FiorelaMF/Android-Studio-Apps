...
import app.acsolutions.ventisysupc.LISTAS.ClasesDocente;
import app.acsolutions.ventisysupc.LISTAS.ClasesDocenteAdaptador;
import app.acsolutions.ventisysupc.LISTAS.DetalleAsistenciaDocente;
import app.acsolutions.ventisysupc.LISTAS.DetalleAsistenciaDocenteAdaptador;
...
public class ClienteDetalleAsistenciaActivity extends AppCompatActivity implements View.OnClickListener{
    private Button BACK;
    private RecyclerView LISTA;
    private String CODIGO,DATO;
    private TextView CUR,SEC;
    private ProgressDialog PROGRESO;
    FirebaseFirestore db = FirebaseFirestore.getInstance();

    ArrayList<String> CLASES = new ArrayList<>();

    ArrayList<String> CODIGI = new ArrayList<>();
    ...
    List<DetalleAsistenciaDocente> clases;
    DetalleAsistenciaDocenteAdaptador adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SEC = findViewById(R.id.seccion);
        Bundle EXTRAS = getIntent().getExtras();
        if (EXTRAS == null){ CODIGO = "NO"; }
        else { CODIGO = EXTRAS.getString("CATO"); }
        Log.e("LEGa: ",CODIGO);
        String[] coso = CODIGO.split("_");
        BuscarDatos(coso[1],coso[2]);
        CUR.setText("Curso: " + CLASES.get(0));
        SEC.setText("Sección: " + coso[3]);

        LISTA.setLayoutManager(new LinearLayoutManager(this));
        clases = new ArrayList<>();
        adapter = new DetalleAsistenciaDocenteAdaptador(clases);
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    for (QueryDocumentSnapshot document : Objects.requireNonNull(task.getResult())) {
                        ...
                        String PP  = coso[0] + "_" + coso[1] + "_" + coso[2] + "_" + coso[3] + "_" + coso[4] + "_" + coso[5];
                        if (CODIGO.equals(PP)){
                    ...
                    PROGRESO.dismiss();
                    Toast.makeText(ClienteDetalleAsistenciaActivity.this, "Error de Conexión", Toast.LENGTH_LONG).show();
                }
                ...
                if (CODIGI.size()>0){
                    for (int I = 0;I<CODIGI.size();I++){
                        DetalleAsistenciaDocente CADENA = new DetalleAsistenciaDocente(CODIGI.get(I),NOMBRE.get(I),INGRES.get(I),FECHAS.get(I),SALIDA.get(I),ESTADO.get(I),TEMPER.get(I),FOTOTO.get(I));
                        clases.add(CADENA);
                    }

                    Log.e("H1: ", CODIGI.toString()+ " " + CODIGI.size());
