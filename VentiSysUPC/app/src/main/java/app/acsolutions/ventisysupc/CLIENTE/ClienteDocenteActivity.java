...
public class ClienteDocenteActivity extends AppCompatActivity implements View.OnClickListener{
    private Button BACK;
    private RecyclerView LISTA;
    private List<ClasesDocente> clases;
    private ClasesDocenteAdaptador adapter;
    private ProgressDialog PROGRESO;
    private TextView TEXTO;
    FirebaseFirestore db = FirebaseFirestore.getInstance();
    String CODIGO,DATO,COSOTA;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_cliente_docente);
        BACK = findViewById(R.id.back);
        LISTA = findViewById(R.id.lista);
        TEXTO = findViewById(R.id.texto);
        LISTA.setLayoutManager(new LinearLayoutManager(this));
        clases = new ArrayList<>();
        adapter = new ClasesDocenteAdaptador(clases);
        LISTA.setAdapter(adapter);
        Intent DETALLES = new Intent(this, ClienteDetalleAsistenciaActivity.class) ;
        ...
        db.collection("asistencias").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            ...
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    PROGRESO.dismiss();
                    Toast.makeText(ClienteDocenteActivity.this, "Error de Conexión", Toast.LENGTH_LONG).show();
                }
                clases.removeAll(clases);
                if (CLASES.size()>0){
                    for (int I = 0;I<CLASES.size();I++){
                        ClasesDocente CADENA = new ClasesDocente(FECHAS.get(I),CLASES.get(I),SECCIO.get(I));
                        clases.add(CADENA);
                    }
        ...
        adapter.setOnItemClickListener(new ClasesDocenteAdaptador.OnItemClickListener() {
            @Override
            public void onItemClick(ClasesDocente clases) {
                String[] COSO = clases.getCURSO().split(": ");
                int k = CLASES.indexOf(COSO[1]);
                String HH = STRING.get(k);
                String[] UU = HH.split("_");
                HH = UU[0]+"_"+UU[1]+"_"+UU[2]+"_"+UU[3]+"_"+UU[4]+"_"+UU[5];
                Log.e("est:" , HH);
                COSOTA = HH;
                Log.e("est:" , COSOTA);
                DETALLES.putExtra("DODO:","HOLAvgjfhgkhgkkghfjghgflgjhgfdzffhgjkjhlkjlklhkjghgdhfgdfsfdghfjgk");
                DETALLES.putExtra("CATO",COSOTA);
