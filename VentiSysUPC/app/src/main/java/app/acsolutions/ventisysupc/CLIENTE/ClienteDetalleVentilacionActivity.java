...
public class ClienteDetalleVentilacionActivity extends AppCompatActivity implements View.OnClickListener{
    public Button BACK;
    private TextView AUL,ASI,VEN,CUR,FEC;

    String CODIGO,DATO,INFO,AULA,VENTI;

    GraphView GRAFICA;
    ArrayList<Integer> HORASIO = new ArrayList<>();
    ArrayList<Integer> VALORES = new ArrayList<>();
    ArrayList<Integer> ASISTEN = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_uso_detalle_ventilacion);
        BACK = findViewById(R.id.back);
        AUL = findViewById(R.id.aula);
        CUR = findViewById(R.id.curso);
        ASI = findViewById(R.id.asistencia);
        VEN = findViewById(R.id.ventilacion);
        FEC = findViewById(R.id.fecha);
        ...
        Log.e("LEGO: ",CODIGO);
        String [] AA = CODIGO.split("_");
        BuscarDatos(AA[1],AA[2]);
        CUR.setText("Curso: " + CLASES.get(0));
        FEC.setText("Fecha: " + AA[4].substring(0,2)+"/"+AA[4].substring(2,4)+"/"+AA[4].substring(4,8));

        db.collection("clases").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if (task.isSuccessful()) {
                    for (QueryDocumentSnapshot document : Objects.requireNonNull(task.getResult())) {
                        ...
                        String HOLA = coso[0]+"_"+coso[1]+"_"+coso[2]+"_"+coso[3]+"_"+coso[4]+"_"+coso[5];
                        if (HOLA.equals(CODIGO)){
                            INFO = document.getData().toString();
                            INFO = INFO.replace("{", "");
                            INFO = INFO.replace(" ", "");
                            INFO = INFO.replace("}", "");
                            ...
                            for (String s : DATOS) {
                                String[] COSO = s.split("=");
                                if ("AULA".equals(COSO[0])) {
                                    AULA = COSO[1];
                                    AUL.setText("Aula: "+COSO[1] );
                                }
                            }
                        }
                    }
                    ...
                    PROGRESO.dismiss();
                    Toast.makeText(ClienteDetalleVentilacionActivity.this, "Error de Conexión", Toast.LENGTH_LONG).show();
                }
            }
        });
    }
    ...
    private void SacarMedidas() {
        db.collection("ventilacion").document(CODIGO)
            ...
            public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                if (task.isSuccessful()) {
                    ...
                    if (Objects.requireNonNull(document).exists()) {
                        ...
                        for (String s : DATOS) {
                            String[] COSO = s.split("=");
                            HORASIO.add(Integer.parseInt(COSO[0]));
                            String[] UU = COSO[1].split("_");
                            VALORES.add(Integer.parseInt(UU[0]));
                            ASISTEN.add(Integer.parseInt(UU[1]));
                        }
                        Collections.reverse(HORASIO);
                        Collections.reverse(VALORES);
                        Collections.reverse(ASISTEN);
                        double PRO  = 0;
                        for (int i = 0; i<VALORES.size();i++){
                            PRO = PRO + Double.parseDouble(VALORES.get(i).toString());
                        }
                        PRO = PRO/VALORES.size();
                        if (PRO <= 600){
                            VENTI = "BUENA";
                        }
                        else if (PRO> 600 && PRO <= 900){
                            VENTI = "REGULAR";
                        }
                        else if (PRO > 900){
                            VENTI = "MALA";
                        ...
                        DataPoint[] VALORESOTE = new DataPoint[VALORES.size()];
                        for (int i = 0; i<VALORES.size(); i++ ){
                            VALORESOTE[i] = new DataPoint(HORASIO.get(i),VALORES.get(i));
                        }
                        DataPoint[] ASISTONTAS = new DataPoint[VALORES.size()];
                        for (int i = 0; i<VALORES.size(); i++ ){
                            ASISTONTAS[i] = new DataPoint(HORASIO.get(i),ASISTEN.get(i));
                        }
                        LineGraphSeries<DataPoint> series = new LineGraphSeries<>(VALORESOTE);
                        LineGraphSeries<DataPoint> series2 = new LineGraphSeries<>(ASISTONTAS);
                        series.setTitle("PPM");
                        series2.setTitle("ASISTENCIA");
                        series.setColor(Color.WHITE);
                        series2.setColor(Color.YELLOW);
                        GRAFICA.setTitle("Gráfica de Ventilación - " + AULA);
                        GRAFICA.setTitleTextSize(70);
                        GRAFICA.getLegendRenderer().setVisible(true);
                        GRAFICA.getLegendRenderer().setTextSize(25);
                        GRAFICA.getGridLabelRenderer().setHorizontalLabelsVisible(true);
                        GRAFICA.getGridLabelRenderer().setVerticalLabelsVisible(true);
                        GRAFICA.getGridLabelRenderer().setHorizontalAxisTitle("Minutos");
                        GRAFICA.getGridLabelRenderer().setVerticalAxisTitleTextSize(60);
                        GRAFICA.addSeries(series);
                        GRAFICA.addSeries(series2);
                        ASI.setText("Asistencia: " + ASISTEN.get(ASISTEN.size()-1) + " Persona(s)");
                        VEN.setText("Ventilación: " + VENTI);
                    }
                    else {
                        PROGRESO.dismiss();
                        Toast.makeText(ClienteDetalleVentilacionActivity.this, "No existe documento", Toast.LENGTH_LONG).show();
                    }
                }
                else {
                    PROGRESO.dismiss();
                    Toast.makeText(ClienteDetalleVentilacionActivity.this, "No se puso conectar satisfactoriamente " +  task.getException(), Toast.LENGTH_LONG).show();
                }
            }
        });
    }
